...
/* split this function up as its getting way to big now with all these extra features */
 ...
 add_action('bbp_template_after_single_topic', 'bbps_add_support_forum_features');
...
/*Surjith - Custom Functions*/

add_action ( 'bbp_new_topic', 'bbp_save_default_status', 10, 1 );

function bbp_save_default_status($topic_id=0) {
	$default = get_option('_bbps_default_status');
	update_post_meta( $topic_id, '_bbps_topic_status', $default );
}
...
function bbps_generate_status_options($topic_id){
	...
	if ($status)
		...
		if( (get_option('_bbps_status_permissions_admin') == 1) && (current_user_can('administrator') || current_user_can('bbp_moderator')) ) { 
	?>
...
function bbps_modify_title($title, $topic_id = 0){
    ...
    //1 is the resolved status ID
	if (get_post_meta( $topic_id, '_bbps_topic_status', true ) == 1)
		echo '<span class="not_resolved"> OPEN </span>';
	//2 is the resolved status ID
	if (get_post_meta( $topic_id, '_bbps_topic_status', true ) == 2)
		echo '<span class="resolved"> CLOSED </span>';
